...
const HELP_GROUPS: HelpGroup[] = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      "The screen icon (top-left) opens a fullscreen map with flight-timer buttons below it, handy for practicing patterns. The crosshair icon follows your live GPS position. The (i) icon opens a drawer with nearby radio frequencies and an airspace-ceiling check, without leaving this view — see \"Airspace & nearby info\" below. The ⚠ icon is engine-out targeting — see \"Engine out\" above. The ↩ icon undoes the last route edit, same as the \"Undo\" button below the waypoint list. The ⇢ icon toggles the ETA bar (see below) on and off. The small dial (bottom-right) rotates the map — drag it to point any direction up; double-click/tap it to reset to north-up. All the map's own buttons (zoom, fullscreen, follow, info, engine-out, undo, ETA bar, layers) are sized for easy use one-handed in flight.",
      ...
      "Open the strip-search dropdown to browse a curated list of grass/gravel strips in Norway and Sweden (including a large batch sourced from the community-maintained \"Norske Mikroflystriper\" map), with surface, runway, length, and elevation shown where confirmed. A small number of paved, major/controlled fields (e.g. Ørland Flystasjon) are included too, purely for their published radio frequencies and as diversion reference points — they're clearly not LSA strips, and controlled-airspace procedures apply, not uncontrolled-field ones. None of this is an official AIP — PPR requirements, restrictions, closures, and even whether a field is still operational can be out of date, so always verify locally before relying on it. For anywhere else, type coordinates directly in decimal degrees (e.g. 44.8848, -93.2223); south and west are negative.",
      "The windsock (bottom-right, above the rotate knob) shows real METAR wind — direction, speed, and gusts if reported — from the nearest reporting station to your live position, or your first waypoint with GPS off. It's the actual published observation, not your manually-entered planning wind from the Wind panel, so it can differ (and is generally more current). The station ICAO and its distance from you are shown underneath; tap ↻ to refresh on demand, or it refreshes itself automatically roughly every 15 minutes and whenever you've moved more than 20nm from where it last checked. \"VRB\" means the station reported variable direction; a grey dot means no usable wind reading yet. Rotating the map (track-up) doesn't affect it — it keeps pointing the true compass direction regardless.",
      "With GPS on and moving, a mint-green dashed bar extends ahead of you along your current track, ticked every 5 minutes out to 30 — \"how far will I get\" rather than fixed distance rings, since time-to-fly is usually the more useful question in the air. Distance per tick comes from your live ground speed once it's above 5kt, falling back to planned cruise speed below that (same convention as the direct-to ETA). It follows track, not planned route, so it bends away from your route line in a crosswind or during a turn — that's expected, it's showing where you're actually headed right now, not where you're trying to go."
    ]
  },
  {
    title: 'Airspace & nearby info',
    paragraphs: [
      "Opened from the (i) icon on the map. \"Radio frequencies\" mirrors the Radio frequencies panel, filtered to within 50nm of your route or live position. \"Airspace ceiling\" is a manual, on-demand check against OpenAIP's structured airspace data — a different, more detailed data source than the raster aviation overlay on the map itself. \"Check at\" defaults to Auto (your live GPS position if tracking, otherwise your first waypoint) but can be pointed at any waypoint along the route instead; tap \"Check airspace\" to run it for whichever one is selected. It reports the highest altitude you could climb to there before entering airspace that needs an ATC clearance (Class A–D) or must be avoided (restricted, danger, prohibited, or a CTR).",
      "With 2+ waypoints, \"Check airspace at every waypoint\" runs the same lookup for the whole route in one go and lists a one-line result per waypoint, rather than checking each one by hand. It queries them one at a time (not all at once) so a long route doesn't hammer OpenAIP, and each row fills in as its result arrives — a slow or failed lookup on one waypoint doesn't hold up the rest.",
      "Either way, this checks a single point directly beneath a waypoint, not a corridor along the leg between them or your actual flight path, and only refreshes when you tap the button — recheck it as you move or as the plan changes. Floors given as \"AGL (approx.)\" are estimated without local terrain data, and flight-level-referenced floors assume standard pressure. OpenAIP's dataset is community-maintained and not exhaustive. This is a planning aid, not a substitute for checking the current AIP and getting an actual clearance before entering controlled airspace."
    ]
  },
  {
    title: 'Live tracking',
    paragraphs: [
      ...
      "While GPS tracking is on, this also tries to keep the screen from sleeping (the Wake Lock API) — the status line here says \"screen kept awake\" when it's actually holding that lock. Some browsers don't support this (older Safari, mainly); if the browser DOES support it but the request itself fails (denied, or reclaimed under battery saver), the specific error is now shown below the status line rather than failing silently, since \"unsupported\" and \"tried and failed\" need different fixes. Either way, your device's own display-timeout setting is the fallback. It releases automatically once you stop tracking."
    ]
  },
  {
    title: 'Daylight',
  ...
  },
  {
    title: 'Bearing & track',
    paragraphs: [
      "A compass dial plus a needle pointing toward a target airfield: pick one from the dropdown, or leave it on \"Auto\" to follow your engine-out target when that's active, otherwise your route's destination. The ring of letters turns like a real magnetic compass card as you turn, N always at true north; the needle points at the target's bearing relative to your nose. Distance, left/right angle, and altitude difference are shown as numbers below the dial, not just on the needle, so they're still there if the needle isn't showing (target not roughly ahead, or no heading source available yet).",
      "Heading normally comes from this device's compass (magnetometer) — iOS will prompt for a one-time \"Motion & Orientation Access\" permission the first time you tap Enable. If the compass isn't giving a usable reading, this falls back to GPS course-over-ground instead (labeled \"Trk\" in the readout, and noted when it's standing in for \"Hdg\"): a genuinely different data source that needs no magnetometer at all, only movement, so it works on devices whose compass doesn't (uncalibrated, or — common on tablets specifically — no magnetometer chip in the first place). If neither is available, this section says so and tells you which case it looks like: no orientation sensor detected at all, versus one present but not giving a calibrated heading yet (try a slow figure-8 motion to calibrate it).",
      "This is not a rendered terrain view and can't know about hills, towers, or anything else between you and the target — just a computed bearing pointer toward known coordinates. There used to be a pitch/bank attitude horizon and a turn coordinator here too; both were dropped — they depended on accelerometer/gyroscope behavior that turned out to need constant per-device recalibration (invert/swap settings, sensitivity to jitter and g-force) for something a real panel instrument already does better, and it wasn't worth that upkeep for a feature that isn't the point of this app."
    ]
  },
  {
