...
const HELP_GROUPS: HelpGroup[] = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      "Push and hold anywhere on empty map space (or right-click on desktop) to open a menu at that point: \"Add waypoint here\" appends it to the end of your route. \"Set as start\" inserts it as the new first waypoint instead, ahead of everything else — handy for planning a route from scratch by tapping your departure point first, then tapping the rest of the route in order with \"Add waypoint here\". \"Direct to here\" behaves differently depending on GPS: with GPS off it's inserted as your immediate next waypoint, ahead of the rest of your plan; with GPS on, it instead draws a dashed blue line straight from your live position to that point (shown with distance and, once your GPS ground speed is above 5kt, an ETA in minutes and an estimated fuel used — from your aircraft's cruise burn rate applied to that ETA, not a measured figure — in the bottom-left HUD — tap it to cancel) without touching your saved route at all. The remove button on each waypoint refuses to go below 2 (nav log and fuel calc need a departure and destination) — use \"Clear all waypoints\" below the list to fully reset the route instead.",
